...
## R5. 관리자 진료과별 격리 (Department-scoped Admin)
- **R5.1**: `users` 테이블에 `department_id` FK 추가 — 관리자는 소속 진료과 1개만 배정
- **R5.2**: 관리자 로그인 시 JWT에 `department_id` 포함, 서버 미들웨어에서 진료과 권한 검증
- **R5.3**: 관리자 API(콘텐츠 CRUD, 환자 관리, 알림 발송)에서 소속 진료과 데이터만 접근 가능
- **R5.4**: 다른 진료과 데이터 접근 시도 시 403 Forbidden 반환
- **R5.5**: 관리자 에디터 UI에서 소속 진료과 콘텐츠만 표시 (드롭다운 불필요 — 자동 필터)
- **R5.6**: 슈퍼관리자(department_id=NULL) 개념은 v2 범위 외 — 모든 관리자는 1개 진료과 전담
